...
function check(gl, x, y, mouseProgram, door, doorTexture) {
  g_picked = true;

  gl.useProgram(mouseProgram);
  gl.uniform1i(mouseProgram.u_Clicked, 1);  // Pass true to u_Clicked
  ...
  gl.uniform1i(mouseProgram.u_Clicked, 0);  // Pass false to u_Clicked(rewrite the cube)
  drawTexDoors(gl, mouseProgram, door, doorTexture);
}
...
function pointDist(a,b){
  if(a.length != b.length){
    console.log('The vectors must have the same length\n' + a.lenght);
